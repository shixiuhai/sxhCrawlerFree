# 获取虎牙直播的真实流媒体地址。
# https://github.com/iceking2nd/real-url/tree/master
import json
import requests
import re
import base64
...
def live(info):
    ...
    s= info["roomInfo"]["tLiveInfo"]["tLiveStreamInfo"]["vStreamInfo"]["value"][0]
    stream_info["flv"]= "{}/{}.{}?{}".format(s["sFlvUrl"], 
                                             s["sStreamName"], 
                                             s["sFlvUrlSuffix"],
                                             process_anticode(s["sFlvAntiCode"], uid,s["sStreamName"])
                                             )
...
def process_anticode(anticode, uid, streamname):
    ...
    q["fm"][0] = base64.b64decode(q["fm"][0]).decode('utf-8').replace("$0", q["uid"][0])\
    .replace("$1",streamname).replace("$2", ss)\
...
def get_real_url(room_id):
    try:
        ...
        if room_info["roomInfo"]["eLiveStatus"] == 2:
            print('该直播间源地址为：')
            name=room_info["roomInfo"]["tLiveInfo"]["sNick"]
