...
def gif(url: str):  # 获取动图链接
    ...
    # 获取Network XHR请求
    xhr_requests = driver.execute_script("return performance.getEntriesByType('resource')")
    xhr_urls = [item["name"] for item in xhr_requests]

    zip_set = set()
    # 输出请求的URL
    for all_url in xhr_urls:
        if ".zip" in all_url:
            zip_set.add(all_url)
    return zip_set


def download(downloadUrl: str, name: str, ):  # 下载函数
    if '.zip' in downloadUrl:
        headers = {
            ...
            # 'range': 'bytes=1104126-1134125',
            'sec-ch-ua-platform': '"Windows"',
        }
    elif '.jpg' in downloadUrl or '.png' in downloadUrl:
        headers = {
        ...
        print('链接异常')
        return
    r = requests.get(url=downloadUrl, headers=headers, stream=True,timeout=5)
    if r.status_code == 200:
        open(f'D:\\pixiv\\{name}{downloadUrl[-4:]}', 'wb').write(r.content)  # 将内容写入图片{picUrl[-4:]}
        print("下载成功")
    else:
        print("下载异常")
        ...
        for z in allUrl:
            ...
            for i_1 in aaa:
                ...
                for i_2 in bbb:
                    ...
                    try:
                        download(downloadUrl=i_2, name=ggbond)
                    except:
                        print("下载失败")
                        ggbond = ggbond - 1
                        continue
